...
#include "sdcard.h"
extern SPI_HandleTypeDef SPI_HandleStruct;
...
static uint8_t SDCARD_ReadR1() {
    ...
    uint8_t tx = 0xFF;
    for(;;) {
        HAL_SPI_TransmitReceive(&SPI_HandleStruct, &tx, &r1, sizeof(r1), HAL_MAX_DELAY);
...
static int SDCARD_WaitDataToken(uint8_t token) {
    ...
    uint8_t tx = 0xFF;
    for(;;) {
        HAL_SPI_TransmitReceive(&SPI_HandleStruct, &tx, &fb, sizeof(fb), HAL_MAX_DELAY);
        if(fb == token)
            break;
...
static int SDCARD_ReadBytes(uint8_t* buff, size_t buff_size) {
    ...
    uint8_t tx = 0xFF;
    while(buff_size > 0) {
        HAL_SPI_TransmitReceive(&SPI_HandleStruct, &tx, buff, 1, HAL_MAX_DELAY);
        buff++;
        buff_size--;
    }
...
int SDCARD_Init() {
    ...
    */
    ...
    for(int i = 0; i < 10; i++) { // 80 clock pulses
        HAL_SPI_Transmit(&SPI_HandleStruct, &high, sizeof(high), HAL_MAX_DELAY);
    }
    ...
    */
...

    {
        ...
        HAL_SPI_Transmit(&SPI_HandleStruct, (uint8_t*)cmd, sizeof(cmd), HAL_MAX_DELAY);
    }
...
int SDCARD_WriteSingleBlock(uint32_t blockNum, const uint8_t* buff) {
    ...
    HAL_SPI_Transmit(&SPI_HandleStruct, &dataToken, sizeof(dataToken), HAL_MAX_DELAY);
    HAL_SPI_Transmit(&SPI_HandleStruct, (uint8_t*)buff, 512, HAL_MAX_DELAY);
    HAL_SPI_Transmit(&SPI_HandleStruct, crc, sizeof(crc), HAL_MAX_DELAY);
...
int SDCARD_WriteEnd() {
    ...
    HAL_SPI_Transmit(&SPI_HandleStruct, &stopTran, sizeof(stopTran), HAL_MAX_DELAY);
